...
smoothScatter(nrfall[samp,7], nrfall[samp,st], pch='.', cex=2, col='gray66', nrpoints=200,
...
margin.scat <- function(stxl=6, stxr=2, sty=7, label.panels=TRUE){
  ...
  if(!is.null(stxl)){
    smoothScatter(nrfall[samp,stxl], nrfall[samp,sty], 
                  pch='.', cex=2, col='gray66', nrpoints=200,
                  transformation=xformer(20,.2,.03,0.33),
...
margin.scat(NULL,3,6) # need 1 x 1 version here, rather than 1 x 2
dev.off()
@

\noindent\includegraphics{\Sexpr{fpath('mscat-6-2-7')}}

\noindent\includegraphics{\Sexpr{fpath('mscat-1-4-7')}}

\noindent\includegraphics{\Sexpr{fpath('mscat-5-3-7')}}

\noindent\includegraphics{\Sexpr{fpath('mscat-null-3-6')}}
...
#new:
load('../00common/mycache/filtered.snps.Chr1-qfiltered.rda')
str(filtered.snps$Data$consistent.snps)
consistent <- filtered.snps$Data$consistent.snps
sum(consistent[[2]])
conswhere <- names(consistent[[2]][consistent[[2]]])
